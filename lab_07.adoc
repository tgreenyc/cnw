:compat-mode:
= Lab 7 - Composing Microservices with Spring Cloud and Netflix OSS
...
spring:
  cloud:
    config:
      server:
        git:
          uri: https://github.com/saurabhguptasg/cnw-config-repo
----
+
These properties tell the Config Server to listen on port `8888` and to source its configuration from the Git repository found at https://github.com/saurabhguptasg/cnw-config-repo.
...
It also disables HTTP basic authentication to be able access the endpoints of this service without addressing application security (not covered in this lab).
